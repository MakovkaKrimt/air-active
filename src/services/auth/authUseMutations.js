...
export default class AuthMutations {
  ...
  static useCheckEmail() {
    return useMutation({
      mutationKey: ["checkEmail"],
      mutationFn: (data) => AuthAxiosQueries.checkEmailExists(data),
    });
  }
  ...
  static useRequestResPwd() {
    return useMutation({
      ...
      mutationFn: (token, email, data) =>
        AuthAxiosQueries.requestResetPwd(token, email, data),
    });
  }
